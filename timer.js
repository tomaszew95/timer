var timerPlugin = document.getElementById("ceros-timer-plugin");
(function() {
    "use strict";
    require.config({
        paths: {
    ...
    require(['CerosSDK'], function (CerosSDK) {
        CerosSDK.findExperience()
            ...
            .done(function(experience){
                ...
                animatedNumbers.on(CerosSDK.EVENTS.SHOWN, function(animated){ animationToggleCallback(animated,'no') })
                animatedNumbers.on(CerosSDK.EVENTS.HIDDEN, function(animated){ animationToggleCallback(animated,'yes') })

                const animationStartedCallback = a => {
                    let textBox = document.getElementById(a.id)
                    if(textBox.getAttribute('allow-timer-animation') == null)
                        textBox.setAttribute('allow-timer-animation', 'yes')
                    ...
                    if(textBox.getAttribute('allow-timer-animation') == 'yes'){
                        ...
                        for(let tag of tags){
                            for(let ar of arr){
                                if(tag.includes(ar.name))
                                    ar.value = tag.slice(ar.name.length, tag.length)
                            }
                        }
                        ...
                        const getNumber = countNum => {
                            ...
                            num = num.toString()
                            if(clock)
                                num = clockFunction(countNum, num, col)
                            ...
                            },
                            {
                                ...
                                step: function(){
                                    ...
                                    if(this.countNumber == timer[2] && endText.value != "")
                                        txt.innerText = number + endText.value
                                },
                                complete: function(){
                                    let number = getNumber(this.countNumber)
                                    if(endText.value != "")
